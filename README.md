...
```
primitive tokens  (@inkdropapp/css)   →   --hsl-neutral-100, --hsl-blue-200, …
        │
...
of `--hsl-<family>-<scale>` triplets (`--hsl-neutral-50` … `--hsl-neutral-950`, plus
...
```css
color: hsl(var(--hsl-blue-200)); /* opaque   */
background: hsl(var(--hsl-blue-200) / 40%); /* 40% alpha */
```
...
- **`--md-*`** — Markdown decorations: code blocks, tables, blockquotes, inline marks:
  ...
  ```css
  --md-codeblock-background-color: hsl(var(--hsl-neutral-600) / 20%);
  --md-table-border-color: var(--bd-bg-highlight);
